...
def get_all_stations_vm2(request):
    f = open(staticfiles_storage.path("dynamic/fixtures/stations.json"))
    data = json.load(f)
...
def get_all_vars_vm2(request):
    f = open(staticfiles_storage.path("dynamic/fixtures/variables.json"))
    data = json.load(f)
...
def download_table_validations(request, id, type):
    ...
    if len(data) <= 0:
        return HttpResponseNotFound()
    stations = read_json(staticfiles_storage.path("dynamic/fixtures/stations.json"))
    variables = read_json(staticfiles_storage.path("dynamic/fixtures/variables.json"))
